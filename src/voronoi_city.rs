...
fn build_voronoi_Cells(
    ...
    let circumcenters: Vec<Vec2> = (0..num_triangles)
        .map(|t| {
            let (i0, i1, i2) = (triangles[t*3] as usize, triangles[t*3 + 1] as usize, triangles[t*3 + 2] as usize);
    ...
    points
        ...
        .map(|(i, original)| {
            if cells[i].len() >= 3 {
                ...
                Vec2::new(centroid.x.clamp(bounds_min.x + 1.0, bounds_max.x * 1.0), centroid.y.clamp(bounds_min.y + 1.0, bounds_max.y - 1.0))
            } else {
                *original
            }
        })
...
impl VoronoiLayout {
    pub fn generate(config: &CityConfig) -> Self {
        ...
        let mut points = (0..config.num_cells)
            .map(|_| {
                Vec2::new(rng.random_range(2.0..size - 2.0), rng.random_range(2.0..size - 2.0))
            })
            .collect::<Vec<_>>();
        ...
        for (i, cell_verts) in cells.iter().enumerate() {
            ...
            let max_height = match block_type {
                BlockType::Downtown => {
                    config.max_building_height * (1.0 - normalized_dist * 0.3)
                }
        ...
        VoronoiLayout { blocks, road_segments }
    }
}
